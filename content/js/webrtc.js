...
io.on('signal', function(data) {
	if (data.user_type == "professor" && data.command == "joinroom") {
		console.log("The professor is in class!");
		if (myUserType == "professor") {
		...
		//Switch to the professor listing
		document.querySelector("#requestProfessorForm").style.display = 'none';
		document.querySelector("#waitingForProfessor").style.display = 'none';
		document.querySelector("#professorListing").style.display = 'block';
	}
	else if (data.user_type == "student" && data.command == "callprofessor") {
		...
		document.querySelector("#professorSignup").style.display = 'none';
		...
		if (message.sdp) {
			rtcPeerConn.setRemoteDescription(new RTCSessionDescription(message.sdp), function () {
				// if offer is being received then we will have to answer
				if (rtcPeerConn.remoteDescription.type == 'offer' && myUserType == "professor") {
...
function startSignaling() {
	...
	// get a local stream, show it in small Video area and add it to be sent
...
muteMyself.addEventListener('click', function(ev){
	console.log("muting/unmuting yourself");
	...
	for (var stream of streams) {
		...
		console.log("Local stream details: " + stream.id);
	}
	ev.preventDefault();
}, false);
